...
const { parseNamespace, expiresIn, SEMI_EXPIRE_WINDOW } = require('./expiry');
...
class NameCache {
  ...
  refreshExpiry(tipHeight) {
    const rows = this.db.prepare('SELECT name, height FROM names').all();
    const upd = this.db.prepare('UPDATE names SET expires_in=?, expired=? WHERE name=?');
    this.db.transaction(() => {
      for (const r of rows) {
        const exp = expiresIn(r.height, tipHeight);
        upd.run(exp, exp != null && exp <= 0 ? 1 : 0, r.name);
      }
    })();
  }
