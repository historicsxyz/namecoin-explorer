...
class NameCache {
  ...
  _headerFilter({ maxHeight = null, hashPrefix = null, ops = null } = {}) {
    let join = '';
    const where = [];
    const params = [];
    if (ops === 'with') {
  ...
  pageHeaders({ limit = 20, offset = 0, maxHeight = null, hashPrefix = null, ops = null } = {}) {
    const n = Math.max(0, Number(limit) || 0);
    const off = Math.max(0, Number(offset) || 0);
    const { sql, params } = this._headerFilter({ maxHeight, hashPrefix, ops });
    return this.db.prepare(
      `SELECT h.* FROM headers h ${sql} ORDER BY h.height DESC LIMIT ? OFFSET ?`
    ).all(...params, n, off);
  }

  countHeaders({ maxHeight = null, hashPrefix = null, ops = null } = {}) {
    const { sql, params } = this._headerFilter({ maxHeight, hashPrefix, ops });
  ...
  difficultySparkSeries(limit = 48) {
    ...
    return this.db.prepare(
      'SELECT height, time, difficulty FROM headers WHERE difficulty IS NOT NULL AND difficulty > 0 ORDER BY height DESC LIMIT ?'
    ).all(n).reverse();
  }
  ...
  opsPerDay(days = 90) {
    ...
    return this.db.prepare(`
      ...
      FROM name_ops
      WHERE time IS NOT NULL AND time > 0 AND op != 'NAME_NEW'
      GROUP BY day
      ORDER BY day DESC
      LIMIT ?
    `).all(n).reverse();
