...
function parseAfterCursor(raw) {
  ...
  if (!Number.isFinite(height) || height < 0 || !name) return null;
  return { height: Math.floor(height), name };
}
...
class NameCache {
  constructor(dbPath) {
    ...
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_names_ns_height ON names(namespace, height DESC, name);
  ...
  _migrate() {
    ...
    try { this.db.exec('CREATE INDEX IF NOT EXISTS idx_names_ns_height ON names(namespace, height DESC, name)'); } catch { /* exists */ }
  }
  ...
  _touchIndex() {
    this._indexGen = (this._indexGen || 0) + 1;
    this._nsMemo = null;
  }
  ...
  deleteNamesNotSyncedSince(ts) {
    const n = this.db.prepare('DELETE FROM names WHERE last_sync < ?').run(ts).changes;
    if (n) this._touchIndex();
    return n;
  }
  ...
  countLive() {
    const tip = this._tipHeight();
    if (tip == null) {
      return this.db.prepare('SELECT COUNT(*) c FROM names WHERE expired=0').get().c;
    }
    const liveMin = tip - NAME_EXPIRY_DEPTH;
    return this.db.prepare('SELECT COUNT(*) c FROM names WHERE height > ?').get(liveMin).c;
  }

  sitemapNames(page = 1, limit = 50000) {
    const n = Math.min(Math.max(Math.floor(Number(limit) || 50000), 1), 50000);
    const p = Math.max(Math.floor(Number(page) || 1), 1);
    const off = (p - 1) * n;
    const tip = this._tipHeight();
    if (tip == null) {
      return this.db.prepare(
        'SELECT name FROM names WHERE expired=0 ORDER BY name LIMIT ? OFFSET ?',
      ).all(n, off);
    }
    const liveMin = tip - NAME_EXPIRY_DEPTH;
    return this.db.prepare(
      'SELECT name FROM names WHERE height > ? ORDER BY name LIMIT ? OFFSET ?',
    ).all(liveMin, n, off);
  }

  countByNamespace() {
    const tip = this._tipHeight();
    const gen = this._indexGen || 0;
    if (this._nsMemo && this._nsMemo.tip === tip && this._nsMemo.gen === gen) {
      return this._nsMemo.rows;
    }
    let rows;
    if (tip == null) {
  ...
  page({ start = '', after = '', limit = 100, ns = null, status = null, sort = 'name' } = {}) {
    const where = [];
    const params = [];
    const tip = this._tipHeight();
    ...
    if (updated) {
      const cur = parseAfterCursor(after);
      if (cur) {
        q += (where.length ? ' AND ' : ' WHERE ') + '(height < ? OR (height = ? AND name > ?))';
        params.push(cur.height, cur.height, cur.name);
      }
      q += ' ORDER BY height DESC, name ASC LIMIT ?';
    } else {
...
module.exports = { NameCache, sqliteBackendError, parseAfterCursor };
