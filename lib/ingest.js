...
class IngestService {
  ...
  async _backfillRecentDifficulty(limit = 48) {
    let rows = [];
    try { rows = this.cache.latestHeaders(limit); } catch { return; }
    for (const h of rows) {
      if (h.difficulty > 0 || !h.hash) continue;
      try {
        const hdr = await this.rpc.call('getblockheader', [h.hash, true]);
        if (hdr && hdr.difficulty != null) {
          this.cache.setHeaderDifficulty(h.height, Number(hdr.difficulty), hdr.time);
        }
      } catch { /* skip */ }
    }
  }
