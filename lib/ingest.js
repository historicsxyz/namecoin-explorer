...
class IngestService {
  ...
  start() {
    ...
    this._running = true;
    this._loopPromise = this._loop().catch((e) => console.error('[ingest]', e.message));
  }
  ...
  stop() {
    ...
    return Promise.all([
      ...
      this._loopPromise || Promise.resolve(),
    ]);
  }
  ...
  tick() {
    if (!this._running && !this._tickPromise) return Promise.resolve();
  ...
  async _doTick() {
    ...
    await this._rememberMining(info);
    ...
    this.catchingUp = false;
    await this._backfillRecentDifficulty(48);
  ...
  async _ingestHeight(height, chainTip) {
    ...
    this.cache.insertBlock({
      header: {
        ...
        difficulty: await this._blockDifficulty(block),
      },
      ops,
      tipHeight: chainTip,
    });
    this.cache.setTip(height, block.hash);
    this._notify({ names: ops.map((o) => o.name).filter(Boolean) });
  }

  _notify(evt) {
    if (!this.onUpdate) return;
    try { this.onUpdate(evt || {}); }
    catch { /* HTTP caches must never break ingest */ }
  }

  async _rememberMining(info) {
    if (info && info.difficulty != null && Number(info.difficulty) > 0) {
      this.cache.metaSet('last_difficulty', String(info.difficulty));
    }
    try {
      const hs = await this.rpc.call('getnetworkhashps', [120]);
      if (typeof hs === 'number' && hs > 0) this.cache.metaSet('last_networkhashps', String(hs));
    } catch { /* optional */ }
  }

  async _blockDifficulty(block) {
    if (block && block.difficulty != null && Number(block.difficulty) > 0) {
      return Number(block.difficulty);
    }
    const hash = block && block.hash;
    if (!hash) return null;
    try {
      const hdr = await this.rpc.call('getblockheader', [hash, true]);
      return hdr && hdr.difficulty != null ? Number(hdr.difficulty) : null;
    } catch {
      return null;
    }
  }

  async _backfillRecentDifficulty(limit = 48) {
    const n = Math.min(Math.max(Number(limit) || 48, 8), 120);
    const want = new Set();
    try {
      const ext = this.cache.headerExtent();
      if (ext && ext.minH != null && ext.maxH != null) {
        for (const h of sampleHeights(ext.minH, ext.maxH, n)) want.add(h);
      }
    } catch { /* empty index */ }
    try {
      for (const h of this.cache.latestHeaders(n)) {
        if (h && h.height != null) want.add(h.height);
      }
    } catch { /* empty index */ }
    for (const height of want) {
      let row = null;
      try { row = this.cache.headerAt(height); } catch { continue; }
      if (!row || row.difficulty > 0 || !row.hash) continue;
      try {
        const hdr = await this.rpc.call('getblockheader', [row.hash, true]);
        if (hdr && hdr.difficulty != null) {
          this.cache.setHeaderDifficulty(height, Number(hdr.difficulty), hdr.time);
        }
      } catch { /* skip */ }
    }
  }
