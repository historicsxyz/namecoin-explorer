...
function showFromCache(row, lastOp) {
  if (!row) return null;
  return {
    name: row.name,
    value: row.value,
    ...
    expired: !!(row.expired),
    txid: (lastOp && lastOp.txid) || null,
  };
}

async function loadNameRecord(rpc, cache, name) {
  try {
    const show = await rpc.nameShow(name);
    return { show, fromCache: false, error: null };
  } catch (e) {
    let cached = null;
    let last = null;
    try { cached = cache.get(name); } catch { /* empty index */ }
    try { if (cached) last = cache.lastOpForName(name); } catch { /* empty index */ }
    if (cached) return { show: showFromCache(cached, last), fromCache: true, error: null };
    return { show: null, fromCache: false, error: e.message };
  }
}

function valuePreview(value, max = 56) {
  const decoded = renderValue({ value });
  if (decoded && Array.isArray(decoded.table) && decoded.table.length) {
    const parts = [];
    for (const row of decoded.table.slice(0, 2)) {
      parts.push(row.label + ': ' + (row.isObject ? '{…}' : String(row.value || '')));
    }
    const s = parts.join(' · ');
