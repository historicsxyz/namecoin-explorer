...
const BLOCK_HTML_PREFIXES = ['s|', 'h|', 'l|', 'm|', 'k|', 'b|', 'o|', 'a|'];
...
function htmlCacheKey(req) {
  ...
  if (p === '/names') {
    ...
    const after = String(qfirst(q, 'after') || '').slice(0, 255);
    const limit = parseLimit(qfirst(q, 'limit'), 50, 50);
    const search = String(qfirst(q, 'q') || '').slice(0, 80);
    return 'l|' + lang + '|' + ns + '|' + status + '|' + after + '|' + limit + '|' + search;
  }
  ...
  if (p === '/blocks') {
    const page = parsePage(qfirst(q, 'page'));
    const limit = parseLimit(qfirst(q, 'limit') || qfirst(q, 'count'), 20, 50);
    const search = String(qfirst(q, 'q') || '').slice(0, 80);
    const ops = String(qfirst(q, 'ops') || '').toLowerCase();
    const range = String(qfirst(q, 'range') || '').toLowerCase();
    return 'b|' + lang + '|' + page + '|' + limit + '|' + search + '|' + ops + '|' + range;
  }
  if (p === '/operations') {
    const page = parsePage(qfirst(q, 'page'));
    const limit = parseLimit(qfirst(q, 'limit'), 20, 100);
    return 'o|' + lang + '|' + page + '|' + limit + '|' + (canonicalizeOp(qfirst(q, 'op')) || '');
  }
  if (p === '/addresses') {
    const page = parsePage(qfirst(q, 'page'));
    const limit = parseLimit(qfirst(q, 'limit'), 20, 50);
    return 'a|' + lang + '|' + page + '|' + limit;
  }
  return null;
}
