...
module.exports = function registerRoutes(app, { rpc, cache, ingest }) {
  ...
  app.get('/tx', async (req, res) => {
    ...
    const { groups, fallback, usedFallback, partialFallback, opsByTxid } = await loadRecentTxs(rpc, cache);
  ...
  app.get('/api/namespace/:ns', async (req, res) => {
    ...
    try {
      rows = cache.page({ start, limit: cap, ns, status });
  ...
  app.get('/api/tx', async (req, res) => {
    ...
    if (usedFallback) {
      sendApiJson(req, res, {
        source: 'index',
        items: (fallback || []).map((o) => ({
          txid: o.txid,
          height: o.height,
          nameOps: countOps(o.txid) || 1,
        })),
      });
      return;
    }
  ...
  app.get('/api/name/*', async (req, res) => {
    ...
    const loaded = await loadNameRecord(rpc, cache, n);
    if (!loaded.show) {
...
async function loadRecentTxs(rpc, cache) {
  let headers = [];
  ...
  return { groups, fallback, usedFallback, partialFallback, opsByTxid };
}
