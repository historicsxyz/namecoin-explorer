...
function seriesFromDifficultyRows(rows) {
  return (rows || []).map((r) => {
    ...
    return (hs && t) ? { t, v: hs, height: r.height } : null;
  }).filter(Boolean);
}
...
async function gatherStats(rpc, cache) {
  ...
  const priceSeries = (market && market.series) || [];
  const priceChart = lineChart(priceSeries, {
    height: 200,
    formatY: (v) => formatUsd(v, v >= 1 ? 2 : 3),
  });
  const priceSpark = lineChart(priceSeries.slice(-90), { height: 44, width: 280, spark: true });
  const hrChart = lineChart(hrSeries, {
    height: 200,
    formatY: (v) => formatHashrate(v),
  });

  const opRows = cache.opsPerDay(90);
  const opSeries = opRows.map((r) => ({ t: Number(r.day) * 86400 * 1000, v: Number(r.n) }));
  ...
  return {
    ...
    priceAxis: priceChart.last ? {
      span: new Date(priceChart.first.t).toISOString().slice(0, 10)
        + ' → ' + new Date(priceChart.last.t).toISOString().slice(0, 10),
      range: formatUsd(priceChart.dataMin, 2) + ' – ' + formatUsd(priceChart.dataMax, 2),
    } : null,
    hrAxis: hrChart.last ? {
      span: new Date(hrChart.first.t).toISOString().slice(0, 10)
        + ' → ' + new Date(hrChart.last.t).toISOString().slice(0, 10),
      range: formatHashrate(hrChart.dataMin) + ' – ' + formatHashrate(hrChart.dataMax),
    } : null,
    opsAxis: opsChart.last ? {
      span: new Date(opsChart.first.t).toISOString().slice(0, 10)
        + ' → ' + new Date(opsChart.last.t).toISOString().slice(0, 10),
      range: compactInt(opsChart.dataMin) + ' – ' + compactInt(opsChart.dataMax),
    } : null,
  };
}

module.exports = { gatherStats, hashrateHistory, seriesFromDifficultyRows, headerTickers };

function spark(points, gid) {
  return lineChart(points, {
    height: 28, width: 72, spark: true, padT: 5, padB: 5, padL: 4, padR: 8, gid,
  });
}
...
function headerTickers(cache) {
  ...
  const lastHr = hrSeries.length ? hrSeries[hrSeries.length - 1].v : null;
  return {
